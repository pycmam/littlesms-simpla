...
<li class="catalog_menu"><a href='sections/{$section->url}'>{$section->name|escape}</a></li>
{/if}
{/foreach}
</ul>
</ul>
...
{foreach item=c from=$categories}
	...
	{foreach from=$c->products item=product}
	  <li  class="catalog_menu"><a href='products/{$product->url}'>{$product->category_name|escape} {$product->model|escape}</a></li>
	{/foreach}
	</ul>
	{/if}
{/foreach}  
...
<li class="catalog_menu"><a href='news/{$n->url}'>{$n->header|escape}</a></li>
{/foreach}
</ul>
</ul>
{/if}
...
<li class="catalog_menu"><a href='articles/{$a->url}'>{$a->header|escape}</a></li>
{/foreach}
</ul>
</ul>
{/if}
