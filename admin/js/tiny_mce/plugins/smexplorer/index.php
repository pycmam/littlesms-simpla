...
<div id="main_left">
	<div id="panel1" class="panel"><script language="javascript" type="text/javascript">document.write(tinyMCEPopup.getLang('smexplorer.panel_folder_caption', '?'));</script></div>
		...
		for ($i = 0; $i < count($FILES); $i++) {
			...
			if (IsFileExt($FILES[$i], "jpg") || IsFileExt($FILES[$i], "jpeg") || IsFileExt($FILES[$i], "gif") || IsFileExt($FILES[$i], "png") || IsFileExt($FILES[$i], "bmp") || IsFileExt($FILES[$i], "tif") || IsFileExt($FILES[$i], "ico")) {
				...
				if ($CONFIG["show_preview"] == 1) {
					...
					$icon = "<div id=\"th".$i."\" class=\"jsmpreview\"></div><img style=\"cursor:pointer;\" src=\"img/icon_image_16x16.png\" border=\"0\" onmouseover=\"jSMP.Show(\'th".$i."\', \'".$SESSION["dir"].$FILES[$i]."\', \'".$a["width"]."\', \'".$a["height"]."\');\" onmouseout=\"jSMP.Close(\'th".$i."\');\" onclick=\"SMExplorer_InsertFile(\'".$SESSION["server"]."\', \'".$SESSION["dir"].$FILES[$i]."\', \'".$SESSION["link_target"]."\');\" />";

					unset($a);
				}
				else {
					// Bilddatei ohne Thumbnail-Anzeige
					$icon = "<img style=\"cursor:pointer;\" src=\"img/icon_image_16x16.png\" border=\"0\" onclick=\"SMExplorer_InsertFile(\'".$SESSION["server"]."\', \'".$SESSION["dir"].$FILES[$i]."\', \'".$SESSION["link_target"]."\');\" />";
				}
			}
			else {
				// Keine Bilddatei
				$icon = "<img style=\"cursor:pointer;\" src=\"img/icon_file_16x16.png\" border=\"0\" onclick=\"SMExplorer_InsertFile(\'".$SESSION["server"]."\', \'".$SESSION["dir"].$FILES[$i]."\', \'".$SESSION["link_target"]."\');\" />";
			}

			if ($SESSION["show_file_menu"] == 1) {
				echo "jSMT.AddData(Array('<input id=\"td_checkbox".$i."\" name=\"td_checkbox".$i."\" class=\"checkbox\" type=\"checkbox\" value=\"0\" onclick=\"SMExplorer_Check(".count($FILES).");\" />', '".$icon."', '<input id=\"fn_input".$i."\" name=\"fn_input".$i."\" class=\"edit\" style=\"width:98%;\" type=\"text\" value=\"".$FILES[$i]."\" title=\"".$FILES[$i]."\" onclick=\"SMExplorer_InputClick(this, ".$CONFIG["rename_file_ext"].");\" onblur=\"SMExplorer_InputBlur(this, \'".$FILES[$i]."\');\" onkeypress=\"SMExplorer_InputEnter(event, this, \'".$FILES[$i]."\', \'".bin2hex(RC4("id=1&".$GET))."\');\">', '<img id=\"i1".$i."\" style=\"cursor:pointer;\" src=\"img/icon_insert_16x16.png\" border=\"0\" title=\"\" onclick=\"SMExplorer_InsertFile(\'".$SESSION["server"]."\', \'".$SESSION["dir"].$FILES[$i]."\', \'".$SESSION["link_target"]."\');\" />', '<img id=\"i2".$i."\" style=\"cursor:pointer;\" src=\"img/icon_delete_16x16.png\" border=\"0\" title=\"\" onclick=\"SMExplorer_DeleteFile(\'".bin2hex(RC4("id=1&".$GET))."\', \'".$FILES[$i]."\');\" />', '<img id=\"i3".$i."\" style=\"cursor:pointer;\" src=\"img/icon_show_16x16.png\" border=\"0\" title=\"\" onclick=\"SMExplorer_View_Show(\'".$SESSION["dir"].$FILES[$i]."\');\" />', '".number_format(@filesize(GetDocumentRoot().$SESSION["dir"].$FILES[$i])/1024 ,2 , ",", ".")."&nbsp;KB', '".date(GetDateFormat().' H:i', @filemtime(GetDocumentRoot().$SESSION["dir"].$FILES[$i]))."'), Array('width:18px; text-align:center;', 'width:18px; text-align:center;', '', 'width:16px; text-align:center;', 'width:16px; text-align:center;', 'width:16px; text-align:center;', 'width:102px; text-align:right;', 'width:122px; text-align:center;'));";
			}
			else {
