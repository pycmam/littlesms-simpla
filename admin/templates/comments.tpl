<div id="inserts_all">
  <!-- Вкладки /-->
  <ul id="inserts">
      <li><a href="index.php?section=Comments" class="on">комментарии</a></li>
      <li><a href="index.php?section=Feedback" class="off">обратная связь</a></li>
  </ul>
  <!-- /Вкладки /-->
                  ...
                  <a href="index.php{$item->delete_get}" class="fl" onclick='if(!confirm("{$Lang->ARE_YOU_SURE_TO_DELETE}")) return false;'><img src="./images/delete.jpg" alt=""/></a>
                    <div class="flxc3"  style='background-color:#F5F5F5; border:1px dashed #E0E0E0;padding:4px;'>
                      <p>
                        ...
                        <a href="http://{$root_url}/products/{$item->product_url}" style='color:black;'>{$item->category|escape} {$item->brand|escape} {$item->model|escape}</a>
                        {$item->date|escape} 
                        </div>
                        <a href="index.php?section={$smarty.get.section}&keyword={$item->ip}" class="tovar_on">{$item->name|escape}</a>
